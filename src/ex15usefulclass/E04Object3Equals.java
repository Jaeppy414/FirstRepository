...
class MyClass{
	...
	toString() : 객체가 가진 데이터를 문자열로 반환해준다.
		print()가 호출될 때 항상 자동으로 호출되어 출력에 도움을 준다.
	 */
	@Override
	public String toString() {
		...
		return ""+ data;
//		return "data=" + String.valueOf(data); // 방법은 둘 중 아무거나
	}
	
	/*
	MyClass객체간의 비교를 위해 equals()를 오버라이딩하여 재정의한다.
	...
	public boolean equals(Object obj) {
		...
		이 때 전달된 객체가 MyClass형이 아니라면 비교의 대상이 될 수 
		없으므로 상속관계 확인을 위해 instanceof연산자를 통해 판단한다.
		 */
		if(obj instanceof MyClass) {
			/*
			매개변수가 MyClass타입이 맞다면 비교를 위해 다운캐스팅한다.
			그렇지 않으면 부모타입으로는 자식 멤버에 접근이 불가능하므로
			아래와 같이 data멤버변수에 접근할 수 없다.
			 */
