...
public class E01WrapperClass2 {
	...
	public static void main(String[] args) {
		...
		try {	
			String money = "120원";
			System.out.println(Integer.parseInt(money));//예외발생
		}
		...
		/*
		실수형태의 문자열을 숫자로 변경할 때는 parseInt()를 사용할 수
		없다. 앞과 동일한 예외가 발생한다.
		 */
...
//		System.out.println(Integer.parseInt(floatString));//예외발생
		...
		codePintAt() : 문자열에서 특정 index에 해당하는 한문자의
			아스키코드를 반환한다.
		 */
		...
		판단하는 프로그램을 작성하시오.
			참고) toCharArray() : 문자열을 char형 배열로 반환해주는
				String클래스에 정의된 메소드. 
		*/
		String whiteString = "주어진 문자열 안에 몇개의 공백문자(스페이스)가 있는 지 판단";
		int whiteCount = 0;
