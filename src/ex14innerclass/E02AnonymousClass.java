...
class Singer{
	...
	클래스를 상속하므로 이와 같이 즉시 오버라이딩이 가능하다.
	 */
//	@Override
...
또한 메서드 앞에는 public abstract가 생력되어 있다.
 */
interface ISinger{
	public void dancing();
}
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		//객체 참조변수 자체를 출력하면 toString()메서드를 통해 반환값이 출력된다.
		...
		그 주소를 할당한다.
		개념적으로 new GirlGroup(){자식영역}; ==> 익명 extends GirlGroup();
		형태로 생각하면 된다.
		...
		할 수 없어 에러가 발생한다.
		접근을 위해서는 다운캐스팅(강제형변환)을 해야 하지만 익명클래스는
		클래스의 이름이 없기 때문에 다운캐스팅도 불가능하다.
