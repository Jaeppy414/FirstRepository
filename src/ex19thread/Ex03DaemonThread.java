...
메인쓰레드가 종료되어도 계속 실행되며ㅡ Dead상태가 되어야 실행이 종료되는
...
class NormalThread extends Thread{
	...
	public NormalThread() {}
	/*
	Thread클래스의 생성자를 호출할 때 전달하는 인수를 통해 해당 쓰레드의
	이름을 지정할 수 있다. 이 때 지정된 이름을 getName()을 통해 반환할 수 있다. //어쩐지 자동생성 되더라
	 */
	public NormalThread(String title) {
		super(title);
	}
	/*
	쓰레드 동작 중 비정상적으로 종료될 때 InterruptedException이 발생할 수 있다.
...
종료될 때까지는 프로그램이 종료된 것은 아니다.
주로 프로그램의 보조역할로 배경음악재생, 로그자동저장 등의 업무를
처리한다. 프로그램이 종료될 때 자동으로 종료되므로 주로 무한루프로 구성한다.
 */
...
//		yt1.setName("First 쓰레드");
		...
		//먼저 독립쓰레드를 생성한 후 seDaemon() 메서드로 데몬쓰레드로
		//만들어준다.
