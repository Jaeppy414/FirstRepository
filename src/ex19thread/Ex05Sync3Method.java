...
class SumMulti2{
	...
	/*
	쓰레드의 동기화
	:syncronized 선언으로 인해 정확한 결과가 나오긴 하지만 코드 실행
	시간이 기존에 비해 길어지게 되는 단점이 있다. 즉 동기화 작업은 성능의
	감소를 초래하므로 필요한 부분에 대해서만 동기화 처리를 해주는 것이 좋다.
	 */
...
//	}
	...
	동기화방법2 : 동기화 블럭(필요한 코드만 동기화한다.)
		=> 해당 프로그램에서는 실행코드가 1줄 밖에 없으므로 동기화 방법에
		따른 성능차이가 거의 없다. 하지만 실행코드가 많은 경우 메서드 전체를
		동기화 하는 것보다는 필요한 코드만 동기화하는 "동기화블럭"이 훨씬
		더 좋은 성능을 보이게 된다.
	 */
...
class MultiAdderThread2 extends Thread{
	...
	이 경우 동기화 처리가 되지 않아 동시에 접근이 되는 경우가 발생하게 되어
	잘못된 결과가 출력된다.
	 */
	public void run() {
