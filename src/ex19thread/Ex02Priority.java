...
class MessageSendingThread extends Thread{
	...
	public void run() {
		//각 쓰레드는 1000씩 반복 출력한다. 이때 우선순위를 출력한다.
...
public class Ex02Priority {
	...
	public static void main(String[] args) {
		...
		/*
		1단계 실행
		: 우선순위를 부여하지 않은 상태로 쓰레드 객체를 생성하면 동일한 우선순위 5
		를 부여받게 된다. 이 때는 CPU가 알아서 작업을 분배한다.
		 */
...
//		MessageSendingThread tr3 = new MessageSendingThread("세번째");
		...
		/*
		2단계 실행
		: Thread클래스의 정적상수를 이용하여 우선순위를 부여한다.
		순서대로 10, 5, 1의 우선순위를 부여하게 된다. 상수 대신 정수로
		부여해도 된다.
		 */
		MessageSendingThread tr1 
				= new MessageSendingThread("1번째", Thread.MAX_PRIORITY);
		MessageSendingThread tr2 
				= new MessageSendingThread("2번째", Thread.NORM_PRIORITY);
		MessageSendingThread tr3 
				= new MessageSendingThread("3번째", Thread.MIN_PRIORITY);
