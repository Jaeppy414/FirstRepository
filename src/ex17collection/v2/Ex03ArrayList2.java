...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		/*
		아래 2개의 컬렉션은 내부적인 자료구조만 다를 뿐 구현방법에는 차이가
...
//		ArrayList<Student> list2 = new ArrayList<Student>();
		...
		list2.add(st3);
		list2.add(st4); //인덱스 3
		//List 컬렉션이므로 중복저장이 가능하다. 인덱스로 구분할 수 있다.
		list2.add(st2); //리스트이므로 중복저장 가능
		
		/*
		일반 for문 사용을 위해 size()메서드로 추가된 객체의 갯수를 반환받는다. 
		...
		while(it2.hasNext()) {
			...
			//만약 이름만 출력하고 싶다면 getter()슬 사용하면 된다.
//			System.out.println(it2.next().getName());
		}
		
		/*
		시나리오]
		컬렉션에 저장된 객체를 이름으로 검색하여 삭제할 수 있는 코드를
		작성하시오. 이 때 반복문은 확장for문을 사용하시오.
		 */
		Scanner scanner = new Scanner(System.in);
		System.out.print("삭제할 이름을 입력하세요: "); //입력한 이름 삭제
		...
		for(Student st : list2) {
			...
			getter를 통해 접근해야 한다.
			삭제를 위해 입력한 이름과 Stydent객체에 저장된 name과
			일치하는 지 확인한 후 해당 객체를 찾을 수 있다.
			 */
		...
		System.out.println("삭제된 객체의 이름: "+ 
				list2.remove(2).getName());// 2번에 있는 거 삭제
		System.out.println("현재 객체수: "+ list2.size());
		//총 3개 삭제되고 2개 남는 게 맞음
