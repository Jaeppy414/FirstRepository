...
하나의 메모리를 생성해 이를 공유해서 사용하고자 할 때 쓰는 패턴이다.

작성방법
1. 생성자의 접근지정자를 private로 선언한다.
2. 클래스 외부에서는 생성자에 접근할 수 없으므로 new를 통해
객체생성은 할 수 없다.
3. 클래스 내부에 정적메서드로 해당 클래스의 참조값을 반환하도록
정의한다. 이 때 주로 getInstance() 메서드로 작성한다.
...
class NoSingleTone{
	int instVar;
	/*
	생성자는 거의 대부분 public으로 선언한다. 이유는 public으로
	선언해야지만 클래스 외부에서 new를 통해 객체를 생성할 수 있다.
	 */
	public NoSingleTone() {}
}
//싱글턴 패턴이 적용된 클래스
class SingleTone{
	...
	메서드영역에 로딩되어 사용할 준비를 마치게 된다.
	 */
	private static SingleTone single = new SingleTone();
	/*
	생성자를 private으로 선언하면 클래스 외부에서는 호출할 수
	없으므로 new를 통해 객체를 생성할 수 없게 된다.
	 */
	private SingleTone() {
		
	}
	/*
	정적메서드로 선언된 getInstance()를 통해 해당 객체(참조값)를
	외부로 반환한다. 이런 함수를 "유틸리티 메서드"라고 한다.
	 */
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		/*
		일바적인 방식의 객체생성이므로 객체를 생성할 때 마다 새로운
...
//		SingleTone nt1 = new NoSingleTone();	//에러발생
		...
		//결국 동일한 객체를 사용한 것이므로 마지막에 할당된 값이 출력된다.
