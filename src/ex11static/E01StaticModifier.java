...
-멤버변수 앞에 static 키위드를 붙이면 정적변수가 된다.
-정적변수는 JVM(자바가상머신)에 의해 프로그램이 시작되는 시점에 초기화
된다.
-main()메서드가 실행되기 전에 이미 초기화되므로 main()메서드 실행시
별도의 선언없이 변수를 사용할 수 있다.

접근방법
-클래스 내부 : 변수명으로 접근할 수 있다.(일반적인 멤버변수와 동일)
-클래스 외부
	1. 객체의 참조변수를 통해 접근한다. (권장사항 아님)
	2. 객체 생성없이 클래스명을 통해 접근한다. (권장사항)
...
class MyStatic{
	...
	멤버변수 : 인스턴스형 멤버변수는 반드시 클래스의 객체를 생성한 후
		참조변수를 통해 접근해야 한다. 즉, 일반적인 멤버변수는 객체를
	...
	정적멤버변수 : 객체를 생성할 필요없이 클래스명으로 바로 접근할 수
		있다. JVM이 프로그램을 시작할 때 이미 메모리(Method영역)에 
		로딩된다.
	 */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		(※참조변수를 통해 접근할거면, 굳이 정적변수를 사용할 이유가 없다.)
		 */
