...
///*
//시나리오]
//5번의 게임을 완료한 후 재시작 여부를 물어볼 때 만약 0, 1이 아닌
//숫자를 입력했을 때 개발자 정의 예외클래스를 통해 예외처리를 해보자.
// */
...
//			잘못입력하는 경우 예외가 발생하게 되고 입력을 위해 누른 엔터키가
//			버퍼에 남게 되어 무한루프에 빠지게 된다. 이 때 nextLine()을 통해
//			엔터키를 제거해야 한다.
//			 */
//			try {
//				user = scanner.nextInt(); //버퍼안에 엔터키 하나가 남아있음. 컴퓨터한테는 하나의 문자임. 계속 뭐가 입력이 된 걸로 간주해서 무한루프로 들어가는거임 
//				//3.승부판단
//				//사용자가 정상적으로 입력한 경우
//				if(!(user<1 || user>3)) {
...
//				/*
//				믄자를 잘못 입력해서 예외가 발생하는 경우에만 엔터키를
//				제거해주면 된다.
//				 */
//				scanner.nextLine(); //nextInt가 입력이 되면 line은 try에서 입력을 못 받음
////				e.printStackTrace();
...
//						else {
//							/*
//							정수입력시 음수나 0과 1이 아닌 값을 입력할 때는
//							컴파일러가 인식하지 못하므로 우리가 직접 확인 후
//							예외객체를 생성하고 throw해야한다.
//							 */
//							ChoiceException ce = new ChoiceException();
//							throw ce;
//						}
