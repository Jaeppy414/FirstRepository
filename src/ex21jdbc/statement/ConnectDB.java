...
/*
상속을 목적으로 생성하는추상클래스. 오라클 DB연결과 자원반납을 담당한다.
추상클래스는 객체를 생성할 수는 없지만 참조변수선언, 상속 등의 기능은
사용할 수 있다.
 */
public abstract class ConnectDB { 
	...
	//생성자1 : 계정 아이디, 패스워드를 매개변수로 받은 후 DB연결
	public ConnectDB(String id, String pw) {
		try {
			...
			con = DriverManager.getConnection(
					...
					id,pw);
			System.out.println("오라클 DB 연결성공99");
		} 
		catch (ClassNotFoundException e) {
			System.out.println("오라클 드라이버 로딩 실패99");
		}
		catch (SQLException e) {
			System.out.println("DB 연결 실패99");
			e.printStackTrace();
		}
		catch (Exception e) {
