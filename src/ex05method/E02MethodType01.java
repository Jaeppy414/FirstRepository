...
Method(메서드, 함수)
	...
	즉 어떤 하나의 업무를 처리하기 위한 모듈(부속품)이라 
	정의할 수 있다. 
규칙
-메서드는 반드시 class 내부에 정의해야 한다. 
-메서드 내부에 또다른 메서드를 정의할 수 없다. 이런 경우
에러가 발생한다. 
-반환값이 없다면 void를 반드시 명시해야 한다. 
-함수명은 변수명과 동일한 규칙으로 작성한다. 

Java에서의 Naming Rule(이름을 지을때의 규칙)
-클래스명 : SimpleFunc => 대문자로 시작하는 Camel case로 작성
-메서드 혹은 변수명 : simpleFunc() 혹은 simpleFunc
	=> 소문자로 시작하는 Camel case
-상수명 : SIMPLE_FUNC => 전체를 대문자로 기술한다. 단 연결되는
	단어는 _(언더바)로 구분한다. 
-패키지명 : simple.func => 전체를 소문자로 기술한다. 연결되는
	단어는 .(닷)으로 구분한다. 

메서드형태1] 매개변수도 없고, 반환값도 없는 케이스
...
public class E02MethodType01 {
	...
	static void returnError() {
		...
		따라서 return을 사용할때에는 반드시 조건문과 같이 
		기술되어야 한다. 단 메서드의 마지막 라인에 기술한다면
		조건문이 없어도 된다. 
		 */
//		return; //아래에서 에러가 발생한다. 		
