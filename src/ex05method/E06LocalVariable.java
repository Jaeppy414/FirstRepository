...
가진다. 또한 스택은 CPU가 메모리의 생성 및 소멸을 주관한다. 
 */
public class E06LocalVariable 
{
	public static void main(String[] args) 
	{
		...
		이름의 변수를 선언할 수 없다. 큰 지역에서 이미 선언되어 메모리에
		상주하고 있기때문이다. 
		 */
	//	int num = 9; //에러가 발생하므로 주석으로 처리한다.
		...
		단, if문의 블럭과 else문의 블럭은 서로 다른 지역이므로 동일한 
		이름의 변수를 선언할 수 있다. 
		 */
		if(scope) {	
			int num = 1;
	...
	크기가 동일한 서로 다른 지역에 정의되었으므로 같은 이름의 변수를
	선언할 수 있다. 
	앞의 main메서드의 if~else문의 블럭도 같은 개념으로 변수를 선언한것이다.
	 */
