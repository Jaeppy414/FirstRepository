...
-객체 간에 형변환이 되려면 반드시 두 클래스 간에 상속관계가 있어야
한다.
-부모타입의 참조변수로 자식 객체를 참조할 수 있으므로 매가변수를
부모타입으로 만들면 하위의 자식 객체를 인수로 받을 수 있다.
-매개변수로 전달되는 인수는 부모타입으로 자동형변환되므로
사용을 위해 기존에 어떤 타입이었는지 확인할 때 사용한다.
 */
...
//	static void wrapBox(GoldPaperBox b) {}
	...
	전달된 인수를 최상위 클래스인 Box타입으로 받고 있기 때문이다.
	-Box타입으로 인수를 받게되면 자동으로 형변환(업캐스팅)되므로
	자식객체의 멤버메서드를 호출할 수 없게 된다.
	-이 때 자식의 멤버메서드를 호출하기 위해 강제형변환(다운캐스팅)이
	필요하다.
	-따라서 전달된 인수의 각각의 타입으로 형변환이 가능한 지 판단한 후
	...
	public static void main(String[] args) {
		...
		/*
		String 객체는 Box객체와는 아무런 관계가 없으므로 인수로
