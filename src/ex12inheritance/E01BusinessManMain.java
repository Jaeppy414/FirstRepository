...
정의하는 것을 말한다. 부모클래스를 자식클래스가 상속할 때는 extends
키워드를 사용한다.
 */
//부모(상위)클래스 : 사람의 일반적인 형태를 추상화한다. (super, 기초)
class Man{
	...
	public void tellYourName() {
		//private 멤버라도 동일 클래스 내에서는 접근할 수 있다.
		System.out.println("내 이름은"+ name + "입니다.");
		
	}
}
//자식(하위)클래스 : Man클래스를 상속받아 회사원을 추상화한다. (sub, 유도)
class BusinessMan extends Man{
	...
	public BusinessMan(String name, String company, String position) {
		...
		만약 super()를 기술하지 않으면 컴파일러에 의해 자동으로
		삽입되어 디폴트 생성자가 호출된다.
		 */
		//부모클래스의 매개변수 1개인 생성자를 호출한다.
		super(name);
		//남은 2개를 통해 나머지 멤버변수를 초기화한다.
	...
	public void tellYourInfo() {
		...
		자식에서도 접근할 수 없다. private멤버는 무조건 자신의 클래스
		내부에서만 접근할 수 있다.
		 */
//		System.out.println("이름은"+ name + "입니다.");
		//부모쪽에서 정의된 public 메서드이므로 접근할 수 있다.
		tellYourName();
	}
}
public class E01BusinessManMain {
	...
	public static void main(String[] args) {
		...
		BusinessMan man1 = new BusinessMan(" 이재필 ", " NAVER ", " 팀장 ");
		BusinessMan man2 = new BusinessMan(" 이재필 ", " DAUM ", " 부장 ");
