...
클래스이다.
이와 같은 클래스를 VO(Value Object)라고 부른다. 즉 값만 가진 객체라는
뜻이다.
 */
class Friend{
	//멤버변수 : 이름, 전번, 주소 기본정보 3가지를 표현
	String name;
	String phone;
	String addr;
	//인수생성자 : 멤버변수 초기화
	...
	public void showAllData() {
		System.out.println("1이름: "+ name);
		System.out.println("1전화번호: "+ phone);
		System.out.println("1주소: "+ addr);
	}
	/*
...
class HighFriend extends Friend{
	...
	public HighFriend(String name, String phone, String addr, String nickname) {
		//자식객체는 부모객체를 초기화할 수 있는 인수까지 모두 받은 후
	...
	super()를 통해 호출하고, 자식에서 확장한 변수를 별도의 print()문을
	통해 출력한다.
	 */
	@Override
	public void showAllData() {
		...
		super.showAllData();
		System.out.println("2별명: "+ nickname);
	}
	/*
	...
	public void showBasicInfo() {
		...
		System.out.println("3별명:"+ nickname);
		System.out.println("3전번: "+ phone);
	}
}
...
class UnivFriend extends Friend{
	...
	public void showAllData() {
		...
		super.showAllData();
		System.out.println("4전공: "+ major);
	}
	@Override
	public void showBasicInfo() {
		System.out.println("==대딩친구==");
		System.out.println("5이름: "+ name);
		System.out.println("5전번: "+ phone);
	}
}
/*
...
class FriendInfoHandler {
	//멤버변수
	/*
	Friend클래스는 Univ, High클래스의 부모이므로 해당 타입으로 배열을
	생성하면 하위 객체를 저장할 수 있다.
	 */
	private Friend[] myFriends;
	/*
	친구정보를 저장한 객체를 생성한 후 배열을 저장할 때 인덱스로 사용할
	...
	public void addFriend(int choice) {
		...
		System.out.print("6이름: "); iName = scan.nextLine();
		System.out.print("6전화번호: ");iPhone = scan.nextLine();
		System.out.print("6주소: "); iAddr = scan.nextLine();

		//입력 선택에 따라 고딩 혹은 대딩으로 분기하여 입력받는다.
		if(choice==1) {
			//고딩을 선택한 경우..
			System.out.print("6별명: "); iNickname = scan.nextLine();
			//객체를 생성한 후 참조변수에 저장한다.
			HighFriend high = new HighFriend(iName, iPhone, iAddr, iNickname);
			/*
			참조값을 객체배열에 추가한다. 이 때 인덱스로 사용할 변수를 후위증가
			하므로 0번 인덱스에 객체가 먼저 저장된 후 1 증가하게 된다.
			 */
			myFriends[numOfFriends++] = high;
		}
		else if(choice==2) {
			//대딩을 선택한 경우
			System.out.print("7전공: "); iMajor = scan.nextLine();
			//객체생성과 동시에 참조값을 객체배열에 추가한다.
			myFriends[numOfFriends++] = 
					new UnivFriend(iName, iPhone, iAddr, iMajor);	
		} 
		//더 이상 실행할 문장이 없다면 해당 메서드는 메모리에서 소멸
		//되고 호출한 지점으로 돌아가게 된다.
		System.out.println("8친구정보 입력이 완료되었습니다.");
	}////end of addFriend
	
	/*
	친구의 정보를 출력하기 위한 멤버메서드
	1.친구정보를 추가할 때 High 또는 Univ객체를 배열에 저장한다.
	2. 이 때 객체는 Friend로 자동형변환(업캐스팅)되어 저장된다.
	3. 정보 출력시 배열에 입력된 객체수만큼 반복하여 각 원소를 통해
		정보(멤버변수에 저장된 값)를 출력한다.
	4. 출력을 위한 멤버메서드는 상속관계에서 오버라이딩 처리되어
		있으므로 참조변수의 영향을 받지 않고 항상 자식객체에 오버라이딩
		정의한 멤버메서드가 호출된다.
	5.즉,저장된 객체는 Friend타입이지만 오버라이딩을 통해 별도의
		형변환이 필요하지 않다. 항상 원하는 정보를 출력할 수 있게 된다.
	 */
	//친구의 전체정보를 출력한다.
	public void showAllData() {
	...
	public void searchInfo() {
		...
		정보의 갯수를 저장한 numOfFriends변수를 사용해야 한다.
		 */
//		for(int i=0 ; i<numOfFriends.length ; i++)
		...
		for(int i=0 ; i<numOfFriends ; i++) {
			...
			경우에만 정보를 출력한다. 이 때 compareTo() 대신에 equals()
			를 사용해도 된다.
			 */
//			if(searchName.compareTo(myFriends[i].name)==0) {
	...
	public void deleteInfo() {
		...
		if(deleteIndex == -1) {
			...
			원소들을 앞으로 하나씩 복사한다.
			numOfFriends는 앞에서 1 차감되므로 마지막 원소는 검색이나
			출력에서 무시된다.
			 */
