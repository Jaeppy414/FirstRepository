...
오버라이딩 : 메서드명, 매개변수, 반환형까지 모두 동일한
	메서드를 상속관계에 있는 하위 클래스에서 재정의한 것을
	말한다.
	이 경우 하위 클래스에서 정의한 메서드가 상위클래스의
...
class A{
	...
	//public A() {}
	public void rideMothod() {
		System.out.println("A의 rideMethod");
	}
	public void loadMethod() {
		System.out.println("A의 rideMothod");
	}
}
class B extends A{
//	디폴트 생성자 생략
...
class C extends B{
//	디폴트 생성자는 컴파일러에 의해 자동삽입된다.
// 	public C(){
// 		super();
//	}
	/*
	rideMothod()는 오버라이딩된 메서드로 상위클래스에 정의된
	...
	/*
	loadMethod()는 오버로딩된 메서드로 상위클래스에 정의된
	...
	가려지게 되므로 C클래스에 정의된 메서드 하나만 남게된다.
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		호출된다. 즉 아래 부분은 모두 C클래스에 생성된 riedMEthod()가
		호출된다.
		즉 오버라이딩은 참조변수의 영향을 받지 않는다.
		 */
		...
		/*
		ref3은 C타입의 참조변수이므로, 객체를 참조할 때 모든 영역에
...
//		int a = 10.1;	//에러발생
		...
		/*
		=>기본자료형의 경우 double이 int보다는 더 큰 타입이므로
