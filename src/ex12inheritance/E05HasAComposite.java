...
: X has a Y => X가 Y를 소유(포함)한다.
여기서는 권총을 소유한 경찰을 표현한다.

Has-A와 같이 소유의 관계를 상속으로 표현하면 클래스 간의 강한 연결고리가
형성되어 권총이 없는 경찰을 표현하기가 힘들어진다.
따라서 이 경우에는 구성(Composition)관계로 표현하는 것이 적합하다.
구성관계란 클래스의 멤버로 다른 객체를 사용하는 것을 말한ㄷ.
 */
//권총을 표현한 클래스
class Gun{
	//탄환수
...
class Police {
	...
	public Police(int bNum, int hCuff) {
		...
		/*
		권총의 보유여부 초기화
		: 만약 총알이 없는 상태라면 보유한 권총이 없는 것으로 표현하기
		위해 멤버변수를 null로 초기화한다.
		참조변수가 null이라는 것은 참조할 객체가 없는 것을 의미한다.
		즉, Heap(힙)영역에 생성된 객체가 아예 없으므로 
		소유하지 않는 것을 표현할 수 있다.
		 */
		if(bNum<=0) {
			gun = null;
	...
	//권총을 발사한다 : 단 소유한 총이 있을 때만 쏠 수 있다.
	public void shut() {
		if(gun == null) {
