...
차이점이라면 매개벼수의 유무와 리턴값 정도이다.

Predicate<T> : 매개변수0, 리턴값 boolean
Supplier<T> : 매개변수X, 리턴값 O
Consumer<T> : 매개변수O, 리턴값 X
Function<T> : 둘 다 있음
 */
public class Ex06Define1Predicate {
	...
	public static void main(String[] args) {
		...
		Predicate<T>
		-사전적 의미 : ~에 근거를 두다.
		-매개변수와 boolean 리턴값이 있는 test() 추상메서드가 정의되어 있다.
		...
		람다식을 정의한다. 매개변수로 Human 객체를 전달하고, 성별을 판단하여
		남자인 경우 true, 여자인 경우 false를 반환한다.
		 */
		Predicate<Human> pre = (Human h) -> {
			/*
			매개변수로 전달된 Human객체의 getter()를 통해 성별을 얻어온 후
			"남"인지 판단한다.
			 */
		...
		컬렉션은 값을 변경할 수 없고 참조만 가능하다.
		 */
		List<Human> list1 = Arrays.asList(p1, p2, p3, p4);
		
		/*
		Predicate<Human> aaa = a -> a.getGender().equals("남");
		이와 같은 람다식을 정의한 후 참조변수인 aaa를 전달하는 것과
		아래처럼 람다식 자체를 매개변수로 작성하는 것은 동일하다.
		 */
		/*
		...
		System.out.println("남자평군: "+ maleAvg);
		
		/*
		위와 같이 매개변수의 타입을 생략하는 것도 가능하지마 가독성이
		떨어지므로 타입정도는 아래의 람다식처럼 명시하는 것이 좋다.
		여기서는 여자인지를 판단한다.
		 */
	...
	public static double avg(Predicate<Human> ph, List<Human> li) {
		int pCount = 0;
		int tCount = 0;
		//컬렉션에 저장된 Humna객체의 갯수만큼 4번 반복한다.
		for(Human h : li) {
