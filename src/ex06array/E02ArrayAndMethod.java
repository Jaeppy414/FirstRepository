...
public class E02ArrayAndMethod {
	...
	public static void main(String[] args) {
		 ...
		 */
		int[] arr = {1, 2, 3, 4, 5};
		//배열변수를 선언하였지만 아직은 참조할 배열 객체가 없는 상태
		int[] ref;
	...
	/*
	매개변수로 전달된 배열의 참조값을 ar이 받게 되므로 해당 메서드에서도
	동일한 배열을 참조하게 된다. 즉 main메서드에서 생성한 배열과 동일한
	배열을 참조하게 된다.
	 */
	private static int[] addAllArray(int[] ar, int addVal) {
		for(int i=0 ; i<ar.length ; i++) {
			ar[i] += addVal;
			/*
			ar[i] = ar[i] + addVal;
			위 복합대입연산자를 풀어서 쓰면 이와 같이 된다. 즉 배열의
