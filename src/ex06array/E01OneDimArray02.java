...
public class E01OneDimArray02 {
	...
	public static void main(String[] args) {
		...
		정수형태의 난수를 생성할때에는 10, 100과 같은 정수를 곱해주면
		된다. 
		 */
		double rndNumber = Math.random();
		System.out.println("생성된난수[실수]:"+ rndNumber);
		//정수와 실수를 연산하면 실수의 결과가 되므로 int로 강제형변환 한다.
		int intNumber = (int)(Math.random()*100);
		System.out.println("생성된난수[정수]:"+ intNumber);

		/*
		난수 생성 방법2
		: Random클래스를 사용한다. 사용법은 Scanner와 동일하게 정수형 
		난수를 생성하고 싶다면 nextInt()를 호출한다.  
		 */
		...
		2.45로 %연산하여 나머지를 구한다. 특정 정수는 45로 나누면 나머지는
			0~44사이로 형성된다. 
		...
		/*
		크기가 6인 배열을 생성해서 난수를 생성하여 각 원소에 입력하고
		이를 출력한다. 중복된 난수가 생성될 수 있고, 정렬 또한 되지 
		않는다. 
		 */
