...
public class E05For {
	...
	public static void main(String[] args) {
		...
		//누적합 저장용변수
		int sum = 0;
		//변수 j가 1부터 100이하까지 반복하는 for문 구성
		for(int j=1 ; j<=100 ; j++) { 
			//증가하는 j를 sum에 누적해서 더한다.
			sum += j;  
		}
		 ...
		 */
		/*
		방법1 : 1~10까지 10번 반복에서 if문을 통해 2의 배수를
		찾은후 누적해서 더한다. 
		 */
		int total = 0;		 
		for(int i=1 ; i<=10 ; i++) {
		...
		/*
		for문의 초기식에서 선언된 변수 j는 for문이 종료되는
		순간 메모리에서 소멸된다. 여기서 사용된 변수 j는
		지역변수(Local variable)라고 한다. 지역변수는 해당
		지역을 벗어나면 즉시 소멸되는 특징을 가지고있다.
		 */
		...
		선언하면 해당변수는 for문의 안쪽(좁은지역)에서 사용
		할수있다. 즉 아래에 선언한 변수 i는 main메서드의 
		지역변수로 선언된다. 
		 */
		int i=0;
		for( ; i<=5 ; i++) {			
