...
//		}
		...
		//if문의 조건은 아래와 같이 boolean값을 반환하는 조건식이면 된다.
		if(num%2==0) { 
			//특정한 정수를 2로 나눈 나머지가 0이라면 짝수로 판단할수있다.
		...
		/*
		조건검사후 실행할 문장이 1개인 경우에는 중괄호를 생략할수있다.
		단, 실행문이 2개 이상이라면 한 문장만 if문에 포함되므로 
		반드시 중괄호를 기술해야한다.  
		 */
		if(num%2 == 0) 
			System.out.println("\n"+ num +"은 짝수이다");
		...
		/*
		아스키코드를 알고있을때의 구현방법
		문자 0과 9는 각각 아스키코드 48, 57 이므로 해당 구간내에 
		있다면 숫자로 판단할 수 있다.
		 */
		...
		/*
		아스키코드를 모를때 구현방법으로 문자 '0'과 '9'를 조건에 
		그대로 기술한다. 해당 문자가 결국 아스키코드를 반환하므로
		위 if문과 동일한 결과를 출력하게된다. 
		 */
